...
persistent fig ax aLines xs ys
if f == 1
    rng("default")
    NumOfPts = 100;
    ...
    aLines = gobjects(NumOfPts,1);
    colors = rand(NumOfPts,3);

    for i = 1:numel(aLines)
        aLines(i) = animatedline("LineWidth",0.05,"Color",colors(i,:));
    end
...
arrayfun(@(a,x,y) addpoints(a,x,y), aLines, xs(:,f), ys(:,f));
